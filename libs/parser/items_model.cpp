...
bool ItemsArea::load(const QString &level_path)
{
	clear();
...
bool ItemsArea::save(const QString& path)
{
	m_src_path = path;
	return save();
}

void ItemsArea::create()
{
	clear();
	m_src_path.clear();
}

void ItemsArea::insert(int row, int column, const Item &item)
{
	m_items[row][column] = item;
}

void ItemsArea::remove(int row, int column)
{
	m_items[row][column] = Item();
}

size_t ItemsArea::columnCount() const
{
	return total_width;
}

size_t ItemsArea::rowCount() const
{
	return total_height;
}

Item ItemsArea::get(int row, int column){
	return m_items[row][column];
}

void ItemsArea::clear()
{
...
QString ItemsArea::getSrc_path() const
{
	return m_src_path;
}

bool ItemsArea::load(const QDomDocument &doc)
{
	bool success_flag = false;
	auto root_elem = doc.firstChildElement();
	if (root_elem.isNull() == false){
		auto curr_elem = root_elem.firstChildElement();
...
std::vector<std::vector<Item> > ItemsArea::getItems() const
{
	return m_items;
}

QDomDocument ItemsArea::toDomDocument()
{
	QDomDocument doc;
	...
	for (size_t row = 0; row < m_items.size(); row++)
	{
		for (size_t column = 0; column < m_items[row].size(); column++)
		{
			...
			if (item.type_name().isEmpty() == false){
				...
				root.appendChild(level_item.toElement());
			}
		}
	}
